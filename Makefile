...
generate_local_coverage_report:
	docker exec -u root -it web pytest -p no:warnings -rA -s --odoo-database=db_test --junitxml=coverage/local/junit.xml --cov-report html:coverage/local/cov.html --cov-report xml:coverage/local/coverage.xml --cov-report annotate:coverage/local/cov_annotate --cov=/mnt/addons/contract_sequence /mnt/addons/contract_sequence/
